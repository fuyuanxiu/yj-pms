...
public class FtpClientImpl implements FtpClientService {
     ...
     * @return
     */
    public ApiResponseResult uploadFiles(String url, int port, String username, String password, String path, String[] filenames, InputStream[] inputs) {
        String rootPath = this.env.getProperty("fs.ftp.rootPath", "");
        if (!StringUtils.startsWith(path, "/")) {
            path = File.separator + path;
        }
        ...
        try {
            ...
            for(InputStream input:inputs){
            	success =  ftp.storeFile(new String(filenames[i].getBytes("gbk"), "iso-8859-1"), input);
            	i++;
            	input.close();
            }
    ...
    public ApiResponseResult download(String url, int port, String username, String password, String path, String fileName) {
        ...
        FTPClient ftp = new FTPClient();
       // FTPClientConfig ftp = new FTPClientConfig(FTPClientConfig.SYST_UNIX);
        try {
            ftp.connect(url, port);
            ApiResponseResult localApiResponseResult1;
            if (!ftp.login(username, password)) {
           ...
           // boolean flag = ftp.retrieveFile(new String((path + File.separator + fileName).getBytes("gbk"), "iso-8859-1"), (OutputStream) bos);
